...
- 運用コマンド
  ...
  - whitelist / blacklist / allban
- ブラックリスト・ホワイトリスト制御
  - サーバーごとに ID 管理
  - ホワイトリストは自動BAN・アンチヌーク除外に利用
  - ブラックリストは入室時に自動BAN
  - 一括BANコマンドで該当ユーザーをまとめて処理
- ログ機能
  - VC接続 / 退出 / 移動
  - メッセージ編集 / 削除
  - サーバー情報変更
...
| `/whitelist` | 管理者 | ホワイトリスト追加/削除/一覧表示 |
| `/blacklist` | 管理者 | ブラックリスト追加/削除/一覧表示 |
| `/allban` | 管理者 | ブラックリスト対象を一括BAN |
| `/punishment-log` | モデレーション | TO/KICK回数と履歴表示 |
| `/dashboard` | 管理者 | ダッシュボードを開き、各ページへ移動 |
| `/audit` | 管理者 | 監査ログ表示 |
| `/join-log` | 管理者 | 入室ログ表示 |
...
- `whitelist` / `blacklist` は ID またはメンションから追加・削除・一覧表示ができます。
- `allban` はブラックリスト登録済みユーザーをまとめてBANします。
- `punishment-log` ではユーザーごとの TO / KICK / BAN の回数と履歴を確認できます。
- `dashboard` でサーバーのダッシュボードを開き、ボタンで概要・アンチヌーク・アンチレイド・認証・ログの画面を切り替えられます。
