...
from bot.utils.settings_store import is_on_list, record_punishment
...
async def _check_nuke(guild: discord.Guild, action_label: str, bot: discord.Client, audit_action: discord.AuditLogAction):
    ...
    if executor.id == guild.owner_id:
        return
    if is_on_list(str(guild.id), "whitelist", executor.id):
        return
    ...
    async def on_member_remove(self, member):
        ...
        try:
            ...
            if executor.id == OWNER_ID or executor.id == member.guild.owner_id:
                return
            if is_on_list(str(member.guild.id), "whitelist", executor.id):
                return
            ...
            if triggered:
                ...
                record_punishment(str(member.guild.id), executor.id, "kick", self.bot.user.id, "アンチヌーク: メンバーキックを繰り返し実行")
