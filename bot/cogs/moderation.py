...
class Moderation(commands.Cog):
    ...
    async def warn(self, interaction: discord.Interaction, user: discord.Member, reason: str = "理由なし"):
        ...
        record_punishment(str(interaction.guild_id), user.id, "warn", interaction.user.id, reason)
    ...
    async def mute(self, interaction: discord.Interaction, user: discord.Member, minutes: int = 10, reason: str = "理由なし"):
        ...
        record_punishment(str(interaction.guild_id), user.id, "mute", interaction.user.id, reason)
    ...
    async def timeout(self, interaction: discord.Interaction, user: discord.Member, minutes: int = 10, reason: str = "理由なし"):
        ...
        record_punishment(str(interaction.guild_id), user.id, "timeout", interaction.user.id, reason)
    ...
    async def kick(self, interaction: discord.Interaction, user: discord.Member, reason: str = "理由なし"):
        ...
        record_punishment(str(interaction.guild_id), user.id, "kick", interaction.user.id, reason)
    ...
    async def ban(self, interaction: discord.Interaction, user: discord.Member, reason: str = "理由なし", delete_days: int = 0):
        ...
        record_punishment(str(interaction.guild_id), user.id, "ban", interaction.user.id, reason)
    ...
    @app_commands.command(name="whitelist", description="ホワイトリストを追加・削除・表示します")
    @app_commands.describe(action="add/remove/list", target="ユーザーIDまたはメンション", reason="理由")
    @app_commands.default_permissions(administrator=True)
    async def whitelist(self, interaction: discord.Interaction, action: str, target: str = "", reason: str = "理由なし"):
        from bot.utils.settings_store import add_to_list, get_settings, normalize_member_id, remove_from_list

        action = action.lower()
        if action == "list":
            members = get_settings(str(interaction.guild_id)).get("whitelist", [])
            if not members:
                return await interaction.response.send_message("ホワイトリストは未登録です。", ephemeral=True)
            lines = [f"<@{member}>" for member in members[:20]]
            embed = discord.Embed(title="ホワイトリスト", description="\n".join(lines), color=0x00CC66)
            if len(members) > 20:
                embed.set_footer(text=f"他 {len(members) - 20} 件")
            return await interaction.response.send_message(embed=embed, ephemeral=True)

        if not target:
            return await interaction.response.send_message("対象のユーザーIDまたはメンションを指定してください。", ephemeral=True)

        user_id = normalize_member_id(target)
        if not user_id:
            return await interaction.response.send_message("有効なユーザーIDまたはメンションを指定してください。", ephemeral=True)

        if action == "add":
            add_to_list(str(interaction.guild_id), "whitelist", user_id)
            return await interaction.response.send_message(f"<@{user_id}> をホワイトリストに追加しました。\nReason: {reason}", ephemeral=True)
        if action == "remove":
            remove_from_list(str(interaction.guild_id), "whitelist", user_id)
            return await interaction.response.send_message(f"<@{user_id}> をホワイトリストから削除しました。", ephemeral=True)

        return await interaction.response.send_message("actionは add / remove / list のいずれかを指定してください。", ephemeral=True)

    @app_commands.command(name="blacklist", description="ブラックリストを追加・削除・表示します")
    @app_commands.describe(action="add/remove/list", target="ユーザーIDまたはメンション", reason="理由")
    @app_commands.default_permissions(administrator=True)
    async def blacklist(self, interaction: discord.Interaction, action: str, target: str = "", reason: str = "理由なし"):
        from bot.utils.settings_store import add_to_list, get_settings, normalize_member_id, remove_from_list

        action = action.lower()
        if action == "list":
            members = get_settings(str(interaction.guild_id)).get("blacklist", [])
            if not members:
                return await interaction.response.send_message("ブラックリストは未登録です。", ephemeral=True)
            lines = [f"<@{member}>" for member in members[:20]]
            embed = discord.Embed(title="ブラックリスト", description="\n".join(lines), color=0xE74C3C)
            if len(members) > 20:
                embed.set_footer(text=f"他 {len(members) - 20} 件")
            return await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        if action == "add":
            add_to_list(str(interaction.guild_id), "blacklist", user_id)
            return await interaction.response.send_message(f"<@{user_id}> をブラックリストに追加しました。\nReason: {reason}", ephemeral=True)
        if action == "remove":
            remove_from_list(str(interaction.guild_id), "blacklist", user_id)
            return await interaction.response.send_message(f"<@{user_id}> をブラックリストから削除しました。", ephemeral=True)

        return await interaction.response.send_message("actionは add / remove / list のいずれかを指定してください。", ephemeral=True)

    @app_commands.command(name="allban", description="ブラックリストに登録されたユーザー/ボットを一括BANします")
    @app_commands.describe(reason="BAN理由")
    @app_commands.default_permissions(administrator=True)
    async def allban(self, interaction: discord.Interaction, reason: str = "ブラックリスト対象"):
        from bot.utils.settings_store import get_settings, normalize_member_id

        members = get_settings(str(interaction.guild_id)).get("blacklist", [])
        if not members:
            return await interaction.response.send_message("ブラックリストに登録されたユーザーはありません。", ephemeral=True)
        ...
        for raw in members:
            user_id = normalize_member_id(raw)
            if not user_id:
                continue
            if int(user_id) == interaction.guild.owner_id:
            ...
            try:
                ...
                await interaction.guild.ban(user, reason=f"[SecurityBot] {reason}", delete_message_days=0)
                succeeded.append(user_id)
            except Exception:
                failed.append(user_id)

        summary = (
            f"ブラックリスト一括BANを実行しました。\n"
            f"成功: {len(succeeded)} / 失敗: {len(failed)} / スキップ: {len(skipped)}"
        )
        if succeeded:
            summary += f"\n対象: {', '.join(f'<@{user_id}>' for user_id in succeeded)}"
        if failed:
            summary += f"\n失敗: {', '.join(f'<@{user_id}>' for user_id in failed)}"
        await interaction.response.send_message(summary, ephemeral=True)

    @app_commands.command(name="punishment-log", description="TO/KICKの回数と履歴を表示します")
    @app_commands.describe(user="対象ユーザー（省略時はサーバー全体のランキング）", limit="表示件数")
    @app_commands.default_permissions(moderate_members=True)
    async def punishment_log(self, interaction: discord.Interaction, user: discord.Member | None = None, limit: int = 10):
        from bot.utils.settings_store import get_punishment_history, get_punishment_rankings, get_punishment_summary

        limit = max(1, min(20, limit))

        if user:
            summary = get_punishment_summary(str(interaction.guild_id), user.id)
            history = get_punishment_history(str(interaction.guild_id), user.id, limit=limit)
            lines = []
            for entry in history:
                action = entry.get("action", "unknown").upper()
                moderator = entry.get("moderator_id")
                moderator_text = f"<@{moderator}>" if moderator else "不明"
                reason = entry.get("reason", "理由なし")
                timestamp = entry.get("timestamp", "")
                lines.append(f"{timestamp[:10]} | {action} | {moderator_text} | {reason}")

            embed = discord.Embed(
                title=f"{user.mention} の記録",
                description=f"TO: {summary.get('timeout', 0)}回 / KICK: {summary.get('kick', 0)}回 / BAN: {summary.get('ban', 0)}回",
                color=0x5865F2,
            )
            embed.add_field(name="履歴", value="\n".join(lines) if lines else "記録なし", inline=False)
            return await interaction.response.send_message(embed=embed, ephemeral=True)

        rankings = get_punishment_rankings(str(interaction.guild_id), limit=limit)
        if not rankings:
            return await interaction.response.send_message("対象が存在しません。TO/KICKの記録はまだありません。", ephemeral=True)

        lines = []
        for row in rankings:
            user_id = row["user_id"]
            lines.append(f"<@{user_id}> | TO {row.get('timeout', 0)} / KICK {row.get('kick', 0)} / BAN {row.get('ban', 0)}")

        embed = discord.Embed(title="TO / KICK / BAN 記録ランキング", description="\n".join(lines), color=0x5865F2)
    ...
    def _dashboard_base_url(self) -> str:
        ...
        if "/auth/callback" in callback_url:
            return callback_url.split("/auth/callback", 1)[0].rstrip("/")
        return callback_url.rstrip("/")

    @app_commands.command(name="dashboard", description="ダッシュボードを開き、各画面へ素早く移動できるようにします")
    @app_commands.describe(guild_id="対象サーバーID（省略時は現在のサーバー）")
    @app_commands.default_permissions(administrator=True)
    async def dashboard(self, interaction: discord.Interaction, guild_id: str | None = None):
        target_id = guild_id or str(interaction.guild_id)
        base = self._dashboard_base_url()
        routes = {
            "overview": f"{base}/dashboard/{target_id}",
            "anti_nuke": f"{base}/dashboard/{target_id}/antinuke",
            "anti_raid": f"{base}/dashboard/{target_id}/antiraid",
            "verify": f"{base}/dashboard/{target_id}/verify",
            "logs": f"{base}/dashboard/{target_id}/logs",
        }
        ...
        for name, url in routes.items():
            ...
            embed.add_field(name=label, value=f"[開く]({url})", inline=True)

        view = discord.ui.View(timeout=180)
        for label, url in [
            ("概要", routes["overview"]),
            ("アンチヌーク", routes["anti_nuke"]),
            ("アンチレイド", routes["anti_raid"]),
            ("認証", routes["verify"]),
            ("ログ", routes["logs"]),
        ]:
