...
class EventLog(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        self.recent_joins.insert(0, {
            "user_id": member.id,
            "guild_id": str(member.guild.id),
            "timestamp": int(member.joined_at.timestamp()) if member.joined_at else int(time.time()),
        })
        self.recent_joins = self.recent_joins[:100]
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        if before.channel is None and after.channel is not None:
            await send_log(
                ...
                type="info",
                title="VC接続",
                description=f"{member.mention} が {after.channel.mention} に接続しました。",
                user=member,
                fields=[{"name": "チャンネル", "value": after.channel.mention, "inline": True}],
            ...
            await send_log(
                ...
                user=member,
                fields=[{"name": "チャンネル", "value": before.channel.mention, "inline": True}],
                guild_id=str(guild.id),
            )
            ...
            await send_log(
                ...
                user=member,
                fields=[
                    {"name": "移動元", "value": before.channel.mention, "inline": True},
                    {"name": "移動先", "value": after.channel.mention, "inline": True},
                ],
                guild_id=str(guild.id),
            )
